(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("request3", [], factory);
	else if(typeof exports === 'object')
		exports["request3"] = factory();
	else
		root["request3"] = factory();
})(this, function() {
...
var RequestCore = function () {
  ...
  _createClass(RequestCore, [{
    ...
    value: function _request(_ref) {
      ...
      // 生成缓存key
      var cacheKey = this._getCacheKey(method, options);
      var that = this;
      if (cache) {
        ...
        if (result) {
          ...
          that._handelResult({ result: result, success: success, fail: fail, setCache: false, expire: expire, cacheKey: cacheKey });
          return;
        }
      }
      if (method === 'ajax') {
        (0, _ajax2.default)(options, function (err, result) {
          that._handelResult({ err: err, result: result, success: success, fail: fail, setCache: cache, expire: expire, cacheKey: cacheKey });
        });
      } else if (method === 'jsonp') {
        (0, _jsonp2.default)(options, function (err, result) {
          that._handelResult({ err: err, result: result, success: success, fail: fail, setCache: cache, expire: expire, cacheKey: cacheKey });
          return;
        });
      } else {
        ...
        webBridge.request(json, function (err, result) {
          that._handelResult({ err: err, result: result, success: _success, fail: fail, setCache: cache, expire: expire, cacheKey: cacheKey });
          return;
        });
      }
    }
    ...
    key: '_getCacheKey',
    value: function _getCacheKey(method, options) {
      var data = {};
      if (options.data) {
        // 防止参数顺序改变 , 造成缓存未击中
        data = Object.keys(options.data).sort(function (a, b) {
          return a - b;
        }).map(function (key) {
    ...
    key: '_handelResult',
    value: function _handelResult(_ref2) {
      var err = _ref2.err,
          result = _ref2.result,
          success = _ref2.success,
          fail = _ref2.fail,
          setCache = _ref2.setCache,
          _ref2$expire = _ref2.expire,
          expire = _ref2$expire === undefined ? 10000 : _ref2$expire,
          cacheKey = _ref2.cacheKey;

      if (!err) {
        if (setCache) {
          console.log('set cache .........', cacheKey);
          _store2.default.set(cacheKey, result, new Date().getTime() + expire);
        }
        success(result);
      } else if (fail) {
        fail(err);
...
var Request = function (_RequestCore) {
  _inherits(Request, _RequestCore);
...
(function (win) {
  ...
  var mobile = {
     ...
     * @param callback
     */
    callAppRouter: function callAppRouter(method, params, callback) {
      var req = {
        'Method': method,
        'Data': params
      };
    ...
    request: function request(params, success) {
...
var storages = [__webpack_require__(13), __webpack_require__(12)];
var plugins = [__webpack_require__(9), __webpack_require__(10)];
...
//# sourceMappingURL=request3.js.map